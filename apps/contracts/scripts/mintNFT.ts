...
async function main() {
    ...
    const contractAddress = "0x849039497Ebb45b8B1285509b3D695E55817b8A0"
    const recipient = signer.address

    const myHTSTokenContract = await ethers.getContractAt("MyHTSToken", contractAddress, signer)
    ...
    if (byteLen > 100) {
        throw new Error(`Metadata is ${byteLen} bytes; must be <= 100 bytes for HTS`)
    }
    ...
    const tx = await myHTSTokenContract["mintNFT(address,bytes)"](recipient, metadata, {
        gasLimit: 350_000
    })
    await tx.wait()
